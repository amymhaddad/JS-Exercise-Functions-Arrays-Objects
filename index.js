...
function getCarInfoByIndex(inventory, index) {
    ...
    return `This is a ${row.car_make} ${row.car_model}`     
}
...
var carInventory = require('./data/inventory.js');

function getLastCarInfo(carInventory) {
    const lastCar = carInventory[carInventory.length-1];
...
function getCarInfoById(carInventory, carId) {
    ...
    let foundCar = '';
    carInventory.forEach(function (row) {
        if (row.id === carId) {
            foundCar += `This is a ${row.car_make} ${row.car_model}`;
        }
    });
    return foundCar;
}
...
function sortCarInventory(carInventory) {
    carInventory.sort(function (carA, carB) {
...
function getModelYears(carInventory) {
    let years = carInventory.map((row, i, carInventory) => {
        return row.car_year;
    });
    return years; 
}
...
*/
function getOlderCars(carInventory, userCarYear) {
    return carInventory.filter(row => row.car_year <= userCarYear);
}
...
function getGermanCars(carInventory) {
    const german = ['Audi', 'BMW', 'Mercedes-Benz', 'Volkswagen'];
    return carInventory.filter(car => german.includes(car.car_make));
  } 
